...
export default function Home(){
  ...
  async function fetchData(){
    setError('');
    if(!address.startsWith('0x')){setError('Enter a valid 0x address');return;}
    try{
  ...
  return(
    ...
    <div className="fixed inset-0 flex items-center justify-center p-6 overflow-auto bg-gradient-to-br from-baseblue to-warppurple">
      <div className="w-full max-w-xl bg-white/10 backdrop-blur-xs shadow-glass rounded-3xl p-8">
        ...
        <button onClick={fetchData} className="w-full py-3 rounded-lg bg-baseblue hover:bg-warppurple transition-colors font-semibold shadow-md text-white">
          Calculate
        </button>
